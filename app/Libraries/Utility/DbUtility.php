...
class DbUtility {
	...
	public static function updateArtwork($asId, $image) {
		Podcast::where("as_id", $asId)->update(["image_url" => $image]);
	}
	
	public static function subscribeUser($podcastId, $userId) {
		subscription::create(["user_id" => $userId, "podcast_id" => $podcastId]);
	}
	
	public static function unsubscribeUser($podcastId, $userId) {
		subscription::where(["user_id" => $userId, "podcast_id" => $podcastId])->delete();
	}
}
