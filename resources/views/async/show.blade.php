<div class="jumbotron">
	...
	@if (!Auth::guest())
		@if($subscribed == false)
		<div>
			<input type="hidden" id="sub-show-id"
				value="{{$podcastId}}" />
			<input type="hidden" id="sub-user-id"
				value="{{Auth::id()}}" />
			<button class="btn btn-default sub-button" id="subscribe">
				Subscribe &nbsp; <span class="glyphicon glyphicon-save" id="sub-btn-icon"></span>
			</button>
		</div>
		@else
		<div>
			<input type="hidden" id="sub-show-id"s
				value="{{$podcastId}}" />
			<input type="hidden" id="sub-user-id"
				value="{{Auth::id()}}" />
			<button class="btn btn-default subbed-button" id="unsubscribe">
				Subscribed &nbsp; <span class="glyphicon glyphicon-ok" id="sub-btn-icon"></span>
			</button>
		</div>
		@endif
	@endif
...
<div class="col-md-12">
	<table class="table table-responsive" id="episode-table">
		...
		<tr>
			<td>{{$episode["title"]}}</td>
			<td>
			<a href="#/"> <span id="play-episode-{{$episode['episode_num']}}"
